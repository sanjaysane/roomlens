...
- **Purpose:** the complete loop a real user lives through, shown once
  per role, nothing skipped: designer onboards → lists a product →
  prospect sends a room photo → designer places the product → prospect
  receives the preview → prospect responds.
- **What it PROVES:** (a) designer onboarding end-to-end (studio name,
  catalog add, price, size class); (b) prospect photo flow with the
  quality gate; (c) preset placement + bigger/smaller adjust; (d)
  localized preview delivery (English AND Marathi) with locale-aware
  pricing; (e) nickname-not-phone-number privacy; (f) the four prospect
  options including "talk to the designer."
- **On screen, in order (all from `docs/evidence/engine-output-2026-09-07.log`):**
  1. Designer onboarding transcript (§1): `hi` → welcome → `1` → "What's
     your studio name?" → "Casa Studio" → Studio home. Designer record:
     `display_name: 'Casa Studio'`.
  2. Catalog add transcript (§2): name → `189.00` → size class → cutout
     photo step (with the corrected line: "the backend saves the raw
     photo as-is — no background removal") → "✅ Added: Aria Chair —
     $189.00". Product record: `price_cents: 18900`, `size_class:
     'floor'`, `default_preset: 'floor-center'`.
  3. Prospect 'Asha' flow (§3): `hi` → `2` → pick designer → nickname
     prompt → "Asha" → photo prompt → `[IMG room_asha]` → designer
     notified ("📸 New room photo from Asha!") → opt-in question → `1`
     → opted in. Prospect record: `nickname: 'Asha'`, `opt_status:
     'opted_in'`.
  4. Marathi prospect flow (§4): `language` → `4` → "✅ भाषा बदलली."
     → Marathi prompts → `[IMG room_meera]` → Marathi confirmation.
     Held on the Marathi strings; natural Marathi, never translated
     word-for-word (per the v2 review's External User lessons).
  5. Studio placement (§5–§6): room list shows "📸 Asha" (nickname, not
     phone); product pick → preset pick (`floor-center`) → preview →
     "1️⃣ Bigger / 2️⃣ Smaller / 3️⃣ Perfect — send to prospect" → `3` →
     the actual composite renders (`viz-asha-aria-chair.jpg`,
     `viz-marathi-user-aria-chair.jpg`) delivered with price strips.
  6. Localized delivery text (§6): English "✨ Here's the Aria Chair in
     YOUR room! 💰 Price: $189.00" with the four options (including
     "4️⃣ I want to talk to the designer"); Marathi "✨ पाहा, तुमच्याच
     खोलीत Aria Chair! 💰 किंमत: ₹189.00" with the Marathi four options.
     Held ≥30 s each — the viewer must finish reading.
  7. Quote + visualization DB records (§7): visualization record with
     `placements: [{'product_id': 3, 'preset': 'floor-center',
     'scale': 1.0}]`, quote `subtotal_cents: 18900`, and the same quote
     rendered both ways: `Aria Chair × 1 — $189.00` (en) /
     `Aria Chair × 1 — ₹189.00` (mr).
  8. The photo quality gate: dark/blurry/small photos trigger a polite
     retake (tests `test_dark_photo_triggers_retake`,
     `test_blurry_photo_triggers_retake`, `test_small_photo_triggers_retake`
     in `docs/evidence/test-run-2026-09-07.log`) — "never a misleading
     overlay."
...
- **On screen:** a disclosure card listing milestones "Known gaps"
  ...
  (3) **Marathi parity partial** — English fallback labels; plus the
  review-carried items: webhook signature validation not implemented,
...
- **On screen:**
  ...
  - The two v1.2 alternatives (business-plan.md §2–§3): **qualified-lead
    ...
    **project take-rate 10–15% via escrow** (**assumption**); the choice
    ...
    growth gates).
  - Unit-economics framing (§6): lead-fee model is a volume game
    (per-lead contribution in the hundreds of rupees / tens of dollars,
    assumption); take-rate model needs few deals but carries escrow
    operations and dispute risk. **India and US modeled separately** —
...
- **On screen:** five bullets: (1) real state machine + compositing
  engine, 92 tests green; (2) prospect photo → preview → quote in the
...
- **On screen / spoken:** (1) approve the MVP milestone scope and done
  ...
  photo→preview completion, ≥15% prospect→designer contact, ≥1 paid
  consultation attributed); (2) commit 2–3 designers with real catalogs
  and agreement to report consultation outcomes; (3) approve the real-
  photo quality-bar work (20+ real photos pre-pilot) as the #1
  de-risking step; (4) keep the monetization decision (lead fee vs
  take-rate) gated on the measured funnel.
...
|---|---|
| 92 tests pass, 1 skipped (Postgres smoke needs live DB) | `docs/evidence/test-run-2026-09-07.log` (last line: `92 passed, 1 skipped … in 10.99s`) |
...
| Quote totals $189.00 (en) / ₹189.00 (mr) from the same quote row | engine-output §7: quote record `total_cents: 18900`, both renderings; `test_quote_lines_text_locale_currency` |
...
| ~35 webhook req/s, 0 failures, single worker (50 users, 2 min) | `docs/SCALE.md` measured table (4,183 POSTs, 0 failures); multi-worker NOT tested ("Honest limits") |
...
| Locales: en/es/hi full parity (85 keys), Marathi partial with English fallback | business-plan.md §1 (measured); tests `test_key_parity`, `test_mr_is_subset_of_en`, `test_mr_falls_back_to_english_for_missing_keys` |
